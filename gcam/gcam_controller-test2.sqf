...
while {_l} do {
    ...
    // Initialize movement variables for each loop
    _acx = 0.0;
    _acy = 0.0;
    _aczm = 0.0;
    ...
    // newly added for looking up and down right and left
    private _lookHorzVal = inputAction "LookLeftRight"; // default right stick horizontal
    private _lookVal     = inputAction "LookUpDown"; // default right stick vertical

    // Debugging: Print values to check if they are being read correctly
    diag_log format ["_lookHorzVal: %1, _lookVal: %2", _lookHorzVal, _lookVal];
    ...
    if (abs _fwdVal > 0.02) then {
        _acx = _acx + (0.04 * CFMOVE * _fwdVal);
    };

    // strafe left/right
    if (abs _strafeVal > 0.02) then {
