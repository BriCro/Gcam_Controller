...
while {_cameraAlive} do {
    ...
    if ((abs _playerInputX > _deadZone) or (abs _strafeVal > _deadZone)) then {
        diag_cameraAliveog format ["[GCam Debug] fwdVal: %1, strafeVal: %2", _playerInputX, _strafeVal];
    };
    ...
    private _playerObjffset = [ _moveSpeed * _strafeInput, _moveSpeed * _forwardInput, 0 ];
