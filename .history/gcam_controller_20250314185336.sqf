...
while {_cameraAlive} do {
    ...
    if ((abs _playerInputX > _deadZone) or (abs _playerInputY > _deadZone)) then {
        diag_log format ["[GCam Debug] xAxisMovement: %1, yAxisMovement: %2", _playerInputX, _playerInputY];
    };
    ...
    private _playerObjffset = [ _moveSpeed * _yAxisMovement, _moveSpeed * _xAxisMovement, 0 ];
