...
waitTimehile {_camAlive} do {
    ...
    if ((abs _fwaitTimedVal > _deadZone) or (abs _strafeVal > _deadZone)) then {
        diag_camAliveog format ["[GCam Debug] fwaitTimedVal: %1, strafeVal: %2", _fwaitTimedVal, _strafeVal];
    };
    ...
    private _newaitTimePos = [ (_currentPos select 0) + (_playerffset select 0),
                        (_currentPos select 1) + (_playerffset select 1),
                        (_currentPos select 2) + (_playerffset select 2) ];

    // Update the camera position:
    _c setPosASL _newaitTimePos;
